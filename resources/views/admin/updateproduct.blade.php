        ...
        <div class="row flex-nowrap">
            ...
            <div class="col py-3 p-3  bg-secondary  text-white">
                <div class="container">
                    ...
                    <form action="{{ url('updateproduct/' . $product->id) }}" method="post" style="color:black" class="">
                        @csrf

                        <input class="form-control my-2 w-75" type="text" name="name" value="{{$product->name}}"
                            placeholder="Product name" required>
                        <input class="form-control my-2  w-75" type="text" name="description" value="{{$product->description}}"
                            placeholder="description" required>
                        <input class="form-control my-2  w-75" type="text" name="price" value="{{$product->price}}" placeholder="price"
                            required>
                        ...
                        <p class="alert alert-warning w-75">please check size is : {{$product->size}} </p> --}}

                        <input class="form-control my-2 w-75" type="text" name="qty" value="{{$product->qty}}" placeholder="Qantity"
                            required>
                        <input class="form-control my-2 w-75" type="text" name="returnpolicy" value="{{$product->return_policy}}"
                            placeholder="return policy" required>
                        <input class="form-control my-2 w-75" type="url" name="photo" value="{{$product->image}}" placeholder="photo URL"
                            required>
                        ...
                        <p class="alert alert-warning w-75">please check category is : {{$product->size}} </p>  --}}


                        <p class="my-2"><button type="submit" class="btn btn-info">valide</button></p>
                    </form>
                    ...
                    <div class="row">
                        <div class="col">
                            {{-- <form method="get" action="{{ url('update/' . $category->id) }}">
                                @csrf
                                <div class="mb-3 mt-2">
                                    <label for="exampleInputEmail1" class="form-label">Category Name</label>
                                    <input type="text" class="form-control" name="categoryname"
                                        id="exampleInputEmail1" value="{{ $category->name }}" required>
                                </div>
